...
def parse_args():
    ...
    parser.add_argument(
        "--mixed_precision",
        help="Weights stay fp32, the forward/backward runs in the chosen dtype. bf16 roughly \
            halves activation memory, which is what lets the batch grow",
        default="no",
...
assert mini_batchsize > 0, (
    f"per_gpu_batch_size ({args.per_gpu_batch_size}) must be at least "
